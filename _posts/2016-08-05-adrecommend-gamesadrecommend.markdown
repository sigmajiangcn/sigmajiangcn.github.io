...
## 业务简介
互联网广告的精准投放，在近十年来取得了蓬勃的发展，现在仍然是一个十分活跃的研究领域。对于互娱来说，游戏广告是信息触达用户的一条重要途径。精准化的广告推送，将有助于营销活动更容易取得成功，例如通过广告定位到处于游戏生命周期各个层次的用户，从而有助于拉取新用户、促进活跃、提升付费转化以及防止流失等。我们最近针对掌盟和王者荣耀手机助手等应用，提供个性化的攻略推荐，在优化用户体验中作了一些尝试。

在攻略推荐过程中，用户和攻略的信息作为预测的特征。预测的过程可以看成一个二分类的问题，即计算用户对攻略的感兴趣程度。一般常用由广义线性模型演变而来的逻辑回归作为训练模型，为了提升模型的表示能力，在特征工程中需要设计高阶交叉特征。在特征提取、离散dummy化、交叉、样本生成以及模型训练等流程进行简要介绍。
...
#### 数据源
一般各个业务方面的攻略存储有差异，例如有放在cdb中或者redis中。攻略推荐的原始数据主要需要下面几个字段：攻略标识、中文标题、作者、攻略内容、发布时间、播放次数（例如视频攻略）等其他字段。攻略内容大都以原始的xml形式给出，例如：
